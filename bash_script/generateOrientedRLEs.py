...
if __name__ == '__main__':
    ...
    rleMatrix = np.array(RLE_to_array(sys.argv[1]))
    orientedRLEs = []
    for _ in range(2):
        for _ in range(4):
            orientedRLEs.append(matrixToRLE(rleMatrix))
            rleMatrix = np.rot90(rleMatrix)
        rleMatrix = np.fliplr(rleMatrix)
    
    print('\n'.join(set(orientedRLEs)))
